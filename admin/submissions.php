<?php
/*
* $Id: submissions.php,v 1.1 2006/03/27 14:51:59 mikhail Exp $
* Module: Snippets
* Version: v1.00
...
switch ($op) {
    case 'view':
        ...
        //if (empty($c)) $c = 1;
        // Display the answer
        $result = $xoopsDB->query('SELECT * FROM ' . $xoopsDB->prefix('quotes') . " WHERE quoteID = $t");
        [$quoteID, $catID, $quotext, $author, $reference, $uid, $submit, $datesub] = $GLOBALS['xoopsDB']->fetchRow($result);

        $result2 = $xoopsDB->query('SELECT name FROM ' . $xoopsDB->prefix('quotecats') . " WHERE catID = '$c'");
        ...
        echo "<table border='0' width='100%' cellspacing='1' cellpadding='2'>";
        echo '<tr>';
        echo "<td class='bg3' colspan='2'><b>&nbsp;" . _MD_QUOTE . ": $quotext</td>";
        echo '</tr>';
        echo "<tr><td class='head'>" . _AM_AUTHOR . ": $submitter";
        echo '<br>' . _AM_PUBLISHED . ": $datesub</td></tr>";
        echo "<td><br>$author<br><br></td>";
        echo '</tr>';
        echo "<tr><td class='even'  align = 'center'><b>&nbsp<a href='submissions.php?op=allow&t=$t&c=$c'>" . _AM_SUBALLOW . "</a> <a href='index.php?op=del&subm=1&t=$quoteID''>" . _AM_SUBDELETE . '</a></b></td></tr>';
        echo "<tr><td class='head' colspan='2' align = 'center'><a href='submissions.php?op=cat'>" . _AM_SUBRETURNTO . '</a></td></tr>';
        echo '</table>';
        exit();
        break;
    case 'allow':
        $xoopsDB->queryF('UPDATE ' . $xoopsDB->prefix('quotes') . " SET submit = '1'  WHERE quoteID = $t");
        $xoopsDB->queryF('UPDATE ' . $xoopsDB->prefix('quotecats') . " SET total = total + 1 WHERE catID = $c");
    ...
    default:
        ...
        $results = $xoopsDB->query('SELECT * FROM ' . $xoopsDB->prefix('quotes') . ' WHERE submit = 0 ORDER BY datesub');
        ...
        if (0 == (int)$totalfiles) {
            ...
            echo "<td align='left' class='bg3' colspan =5><b>" . _AM_QUOTEVAL . '</b></td>';
            echo '</tr></table>';
            echo '<br>';
            ...
            while (list($quoteID, $catID, $quotext, $author, $reference, $uid, $submit, $datesub) = $xoopsDB->fetchRow($results)) {
                if ($uid) {
                    $user = new xoopsUser($uid);
                ...
                echo "<td class='head' align = 'center'>$quoteID</td>";

                echo "<td class='even'><a href='submissions.php?op=view&t=$quoteID&c=$catID'>$quotext</a></td>";

                echo "<td class='even'><p align='center'>$submitter</td>";

                echo "<td class='even'><p align='center'>$datesub</td>";

                echo "<td align='center' class='even' > <a href='submissions.php?op=allow&t=$quoteID&c=$catID'>" . _AM_SUBALLOW . '</a></td>';

                echo "<td align='center' class='even' > <a href='index.php?op=del&subm=1&t=$quoteID'>" . _AM_SUBDELETE . '</a>';
