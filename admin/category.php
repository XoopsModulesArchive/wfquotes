<?php
/*
* $Id: category.php,v 1.1 2006/03/27 14:51:59 mikhail Exp $
* Module: Snippets
* Version: v1.00
...
function editcat($catid = '')
{
    ...
    if ($modify) {
        $result = $xoopsDB->query('SELECT name, description, weight, groupid FROM ' . $xoopsDB->prefix('quotecats') . " WHERE catID = '$catid'");
...
switch ($op) {
    ...
    case 'addcat':
        ...
        if ('0' == $_POST['modify']) {
            if ($xoopsDB->query('INSERT INTO ' . $xoopsDB->prefix('quotecats') . " (catID, name, description, weight, groupid, total) VALUES ('', '$name', '$description', '$weight', '$groupid', '0')")) {
                redirect_header('category.php', 1, _AM_CATCREATED);
            } else {
                redirect_header('category.php', 1, _AM_NOTUPDATED);
            }
        } else {
            if ($xoopsDB->queryF('UPDATE ' . $xoopsDB->prefix('quotecats') . " SET name = '$name', description = '$description', weight = '$weight', groupid = '$groupid' WHERE catID = '$catid'")) {
    ...
    case 'del':
        ...
        if ($confirm) {
            $xoopsDB->query('DELETE FROM ' . $xoopsDB->prefix('quotecats') . " WHERE catID = '$catid'");

            $xoopsDB->query('DELETE FROM ' . $xoopsDB->prefix('quotes') . " WHERE catID = '$catid'");

            redirect_header('category.php', 1, sprintf(_AM_CATISDELETED, $quotext));

            exit();
        }  
            $catid = $_POST['catid'];
            $result = $xoopsDB->query('SELECT name FROM ' . $xoopsDB->prefix('quotecats') . " WHERE catid = '$catid'");
            ...
            echo '<table><tr><td>';
            echo myTextForm('category.php?op=del&catid=' . $_POST['catid'] . "&confirm=1&quotext=$name", _AM_YES);
            echo '</td><td>';
    ...
    default:
        ...
        global $xoopsUser, $xoopsUser, $xoopsConfig, $xoopsDB;
        echo '<div><h3>' . _AM_QUOTEADMINCATH . '</h3></div>';

        quotelinks();

        $result = $xoopsDB->query('SELECT * FROM ' . $xoopsDB->prefix('quotecats') . '');
        ...
        if ($check > 0) {
            ...
            $mytree2 = new XoopsTree($xoopsDB->prefix('quotecats'), 'catid', '0');
